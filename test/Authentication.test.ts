...
import { TwoFactorProviderInterface } from "../src/providers/two-factor/TwoFactorProviderInterface"
...
describe("Authentication", () => {
  ...
  describe("twoFactorAuthenticationLogin", () => {
    ...
    it("should throw when a nonexistent 2FA provider is provided", async () => {
      const loginPayload: LoginData = {
        email: "foo@bar.com",
        password: "foobar",
        twoFactorAuthenticationData: {
    ...
    it("should throw when an invalid 2FA provider is provided", async () => {
      const registrationPayload: RegistrationData = {
        username: "FooBar",
        firstName: "Foo",
        lastName: "Bar",
      ...
      const invalidLoginPayload: LoginData = {
        email: "foo@bar.com",
        password: "foobar",
        twoFactorAuthenticationData: {
          provider: "foo",
      ...
      try {
        await authentication.twoFactorAuthenticationLogin(invalidLoginPayload, result.twoFactorUsers)
      } catch (error) {
        expect(error).toBeInstanceOf(AuthenticationError)
        expect(authentication.twoFactorAuthenticationLogin).toHaveBeenCalledTimes(1)
        expect(authentication.verifyTwoFactorUser).toHaveBeenCalledTimes(0)
  ...
  describe("GlobalAuthConfig", () => {
    ...
    it("should get the GlobalAuthConfig object correctly when the getter is user", () => {
      let result = authentication.GlobalAuthConfig

      expect(result).toBeUndefined()
    })

    it("should set the GlobalAuthConfig correctly when the setter is used.", () => {
      authentication.GlobalAuthConfig = {}

      expect(authentication.GlobalAuthConfig).toBeDefined()
    })
  })

  describe("TwoFactorProviders", () => {
    beforeEach(async () => {
      authentication = new Authentication()
      postgresAdapter = new PostgresAdapter()
      await postgresAdapter.setupConnectionWithConnectionString("postgres://postgres:postgrespw@127.0.0.1:5432/felony_auth_test")
    ...
    it("should set the 2FA providers correctly when setter with valid data is used", () => {
      let twoFactorProviders = new Map<string, TwoFactorProviderInterface>()
      let totpTwoFactorProvider = new TOTPTwoFactorProvider()
      ...
      expect(authentication.TwoFactorProviders).toBeDefined()
      expect(authentication.TwoFactorProviders.get(totpTwoFactorProvider.provider)).toBeDefined()
    })
  })
})